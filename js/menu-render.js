document.addEventListener("DOMContentLoaded", () => {
  ...
  const fmt = (p) => (Number.isInteger(p) ? p : p.toFixed(2)).toString().replace(".", ",");

  function renderGroup(group, filterText) {
        ...
        <div class="menu-items">
          ${items
            ...
            <div class="menu-item">
              ...
              <span class="menu-item-price">${fmt(it.p)} <small>RON</small></span>
            </div>`
            )
            .join("")}
        </div>
  ...
  function render(activeId, filterText) {
    ...
    MENU_GROUPS.forEach((group) => {
      ...
      html += renderGroup(group, filterText);
    });
    panelEl.innerHTML =
